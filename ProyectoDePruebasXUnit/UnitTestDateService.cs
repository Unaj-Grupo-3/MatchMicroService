...
{
    public class UnitTestDateService
    {
        [Fact]
        public async void CreateDateTest()
        {
            //ARRANGE

            var mockCommands = new Mock<IDateCommands>();
            var mockQueries = new Mock<IDateQueries>();
            var mockUserApiService = new Mock<IUserApiServices>();
            DateServices dateServices = new(mockCommands.Object, mockQueries.Object, mockUserApiService.Object);

            DateRequest2 request = new()
            {
                MatchId = 1,
                Location = "loc",
                Description = "desc",
    ...
    mockQueries.Setup(query => query.GetAllMatch()).Returns(Task.FromResult(resultExpected));
            ...
            //ACT
            var result = await dateServices.CreateDate(request);
